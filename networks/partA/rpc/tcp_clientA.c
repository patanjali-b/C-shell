...
int main()
{
    ...
    // Create socket
    clientA_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (clientA_socket == -1)
    {
    ...
    // Connect to server
    int connection_status = connect(clientA_socket, (struct sockaddr *)&server_address, sizeof(server_address));
    ...
    while (playAgain == 'Y' || playAgain == 'y')
    {
        ...
        int bytes_sent = send(clientA_socket, &choice, sizeof(choice), 0);
        if (bytes_sent == -1)
        {
        ...
        char result;
        int bytes_received = recv(clientA_socket, &result, sizeof(result), 0);
        if (bytes_received == -1)
        {
        ...
        int playAgainSent = send(clientA_socket, &playAgain, sizeof(playAgain), 0);
        char playAgainReceived;
        int playAgainReceivedBytes = recv(clientA_socket, &playAgainReceived, sizeof(playAgainReceived), 0);
